  ...
  <div class="admin">
    ...
    <main class="admin__main">
      ...
      <p>
        ...
        <a href="https://mxb.at/blog/css-grid-admin-dashboard/">this blogpost</a>.
      </p>
    </main>
...
export default {
  computed: {
    ...
    superAdmin() {
      //   console.log(
      //     this.$store.getters.super.access_list[1].menus[1].mobile_menu_name
      //   );
      //   this.$store.getters.super.access_list.map(item => {
      //     console.log(item);
      //   });
      var superData = this.$store.getters.super;
      let result = [];
      var data = "";
      for (let i = 0; i < superData.access_list.length; i++) {
        for (let j = 0; j < superData.access_list[i].menus.length; j++) {
          result.push(superData.access_list[i].menus[j].mobile_menu_name);
        }
      }

      for (let index = 0; index < result.length; index++) {
        if (result[index] !== "superadmin") {
          data = "Admin";
        } else {
          data = "Super Admin";
        }
